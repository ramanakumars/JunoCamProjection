...
PJ_search = {'class': "modal_partial_trigger modal_gallery_item"}
# the metadata/image download link has this class
zip_search = {'class': "marR download_zip"}
...
def get_data_perijove(PJ):
    ...
    for ID in IDs:
        ...
        with urllib.request.urlopen(sub_url) as response:
            ...
            header = re.findall(heading_pattern, responseText)[0]
            ...
            if ("BLUE, GREEN, RED" in responseText):
                print("%s => %s"%(header, sub_url))
                lines = responseText.split('\n')
                
                for line in lines:
                    # get the zip 
                    zipi = get_tag_from_text(line, 'href', contains=zip_search)
                    ...
                    for zipii in zipi:
                        if zipii not in zips:
                            ...
                            output = subprocess.run(["wget", "-NS", "--content-disposition", "-P", "PJ%d/"%PJ, base_url+zipii])
