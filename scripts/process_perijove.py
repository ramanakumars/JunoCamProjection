...
parser.add_argument("--metadata_folder", help="Path to folder containing metadata files for the perijove", type=str, required=True)
parser.add_argument("--image_folder", help="Path to the folder containing the JPG files for the perijove", type=str, required=True)
parser.add_argument("--backplane_folder", help="Path to store the intermediate backplane information", type=str, required=True)
parser.add_argument("--map_folder", help="Path to store the cylindrically projected maps", type=str, required=True)
parser.add_argument("--map_resolution", help="Map resolution [pixels per degree]", type=int, default=25)
parser.add_argument("--num_processes", help="Number of processors to use for projection", type=int, default=1)
parser.add_argument("--kernel_folder", help="Path to store SPICE kernels", type=str, default="./kernels")
...
for file in files:
    ...
    np.save(os.path.join(args.map_folder, f'{fname}_map.npy'), pc_data.image / pc_data.image.max())
